...
2. Resistance = top N call OI strikes above price  → watch PUT volume clustering there.
3. Support    = top N put  OI strikes below price  → watch CALL volume clustering there.
...
def compute_oi_levels(
    ...
    ----------
    chain : dict
        Output of WebullClient.get_option_chain().
    underlying_price : float
        Current mark price of the underlying.
    ...
    calls_top = _top_by_oi(calls_near, top_n)
    puts_top  = _top_by_oi(puts_near,  top_n)

    levels: list[dict] = []

    for rank, c in enumerate(calls_top, start=1):
        levels.append({
    ...
    for rank, p in enumerate(puts_top, start=1):
        levels.append({
            'level_type':    'SUPPORT',
            'rank':          rank,
            'strike':        p['strike'],
    ...
    logger.info(
        ...
        price, len(calls_top), len(puts_top), atm_range_pct * 100,
    )
    return levels
...
def get_top_oi_snapshot(
    ...
    Used for the daily OI_Snapshot sheet — separate from the computed levels.
    """
...
def _top_by_oi(contracts: list[dict], n: int) -> list[dict]:
    """Deduplicate by strike (keep highest OI), then return top-n by OI."""
